...
type ToolMode = typeof TOOL_MODE_SUMMARY | typeof TOOL_MODE_FULL;
...
function createSystemPromptToolResult(
  mode: ToolMode,
  confirmLargePayload: boolean,
  ...
  try {
    ...
    if (mode === TOOL_MODE_FULL && !confirmLargePayload) {
      return buildToolErrorResult(
        "Large payload blocked by default.",
        "Set confirmLargePayload=true to receive the complete system prompt text.",
        true,
        {
          mode,
          previewLineCount,
    ...
    return buildToolErrorResult(
      `Failed to read system prompt: ${toErrorMessage(error)}`,
      "Verify that system_prompt.md exists at the repository root.",
      false
    );
  }
}

function createBirdStatsServer(): McpServer {
  const server = new McpServer(
    ...
    },
    {
      ...
      instructions:
        "Use summary mode first to minimize token usage. Request full payloads only when required for downstream automation.",
    }
  );
  ...
  server.registerTool(
    ...
    async ({ mode, confirmLargePayload, previewLineCount }) =>
      createSystemPromptToolResult(mode as ToolMode, confirmLargePayload, previewLineCount)
  );
  ...
  server.registerTool(
    ...
    async ({ mode, confirmLargePayload, maxPaths, pathPrefix }) =>
      createApiSpecToolResult("ebird", EBIRD_SPEC_PATH, mode as ToolMode, confirmLargePayload, maxPaths, pathPrefix)
  );
  ...
  server.registerPrompt(
    "check-bird",
    {
      ...
      description:
        "Prompt template for strict BirdWeather + eBird analysis. Required argument: query. Optional argument: token for BirdWeather station auth.",
      argsSchema: {
    ...
    async ({ query, token }): Promise<GetPromptResult> => {
...
function toJsonRpcError(code: number, message: string) {
  return {
    jsonrpc: "2.0",
    error: {
      code,
...
async function startHttpServer(config: RuntimeConfig): Promise<{ close: () => Promise<void> }> {
  ...
  console.error(
    `[birdstats] streamable-http listening on http://${config.host}:${resolvedPort}${config.path}`
  );
